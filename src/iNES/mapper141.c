...
static	void	Sync (void)
{
	...
	if (Mapper.Mirror & 1)
	{
		EMU->SetCHR_ROM2(0,(Mapper.CHRH << 4) | (Mapper.CHRL0 << 1) | 0);
		EMU->SetCHR_ROM2(2,(Mapper.CHRH << 4) | (Mapper.CHRL0 << 1) | 1);
		EMU->SetCHR_ROM2(4,(Mapper.CHRH << 4) | (Mapper.CHRL0 << 1) | 0);
		EMU->SetCHR_ROM2(6,(Mapper.CHRH << 4) | (Mapper.CHRL0 << 1) | 1);
	}
	else
	{
		EMU->SetCHR_ROM2(0,(Mapper.CHRH << 4) | (Mapper.CHRL0 << 1) | 0);
		EMU->SetCHR_ROM2(2,(Mapper.CHRH << 4) | (Mapper.CHRL1 << 1) | 1);
		EMU->SetCHR_ROM2(4,(Mapper.CHRH << 4) | (Mapper.CHRL2 << 1) | 0);
		EMU->SetCHR_ROM2(6,(Mapper.CHRH << 4) | (Mapper.CHRL3 << 1) | 1);
	}
	if (!ROM->INES_CHRSize)
		EMU->SetCHR_RAM8(0,0);
