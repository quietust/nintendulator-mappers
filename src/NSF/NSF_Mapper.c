...
stop:		JSR silence	;silence extra channels, kill play timer and silence internal sound
reset:		SEI
		LDX #$FF
...
irq:		PHA
		...
		BEQ init
		DEX
		BEQ stop	;"Proper" way to play a tune
...
.module	silence
silence:	LDA $3E13	;X=0 coming in, must also be coming out
		LSR A
		BCC _1
		STX $9002
		STX $A002
